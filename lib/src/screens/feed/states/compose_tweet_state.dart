...
class ComposeTweetState extends ChangeNotifier {
  ...
  String description = '';
  String serverToken;
  final String usernameRegex = r'(@\w*[a-zA-Z1-9]$)';
  ...
  bool get displayUserList {
    final RegExp regExp = RegExp(usernameRegex);
    final bool status = regExp.hasMatch(description);
  ...
  void onDescriptionChanged(String text, SearchState searchState) {
    ...
    final String last = text.substring(text.length - 1, text.length);
    ...
    final RegExp regExp = RegExp(usernameRegex);
    final bool status = regExp.hasMatch(text);
    if (status) {
      final Iterable<Match> _matches = regExp.allMatches(text);
      final String name =
          text.substring(_matches.last.start, _matches.last.end);
  ...
  String getDescription(String username) {
    final RegExp regExp = RegExp(usernameRegex);
    final Iterable<Match> _matches = regExp.allMatches(description);
    final String name = description.substring(0, _matches.last.start);
  ...
  Future<void> sendNotification(Feed model, SearchState state) async {
    const String usernameRegex = r'(@\w*[a-zA-Z1-9])';
    final RegExp regExp = RegExp(usernameRegex);
    final bool status = regExp.hasMatch(description);
    ...
    if (status) {
      ...
      getFCMServerKey().then((void val) async {
        /// Reset userlist
        ...
        await Future.forEach(_matches, (Match match) async {
          final String name = description.substring(match.start, match.end);
          if (state.userlist.any((User x) => x.userName == name)) {
            /// Fetch user model from userlist
            /// UserId, FCMtoken is needed to send notification
            final User user =
                state.userlist.firstWhere((User x) => x.userName == name);
